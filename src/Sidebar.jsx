...
const NAV_ITEMS = [
  ...
  { id: "nueva",     icon: "🚀", label: "Nueva Retro", roles: ["admin","owner","temporal"]          },
...
export function Sidebar({ user, view, setView, onLogout, onNuevaRetro }) {
  const teamName = DEMO_TEAMS.find(t => t.id === user.teamId)?.name || "Global";
  const items = NAV_ITEMS.filter(i => i.roles.includes(user.role));

  return (
    <aside style={{ width:210, flexShrink:0, background:"#111318", borderRight:"1px solid rgba(255,255,255,.06)", display:"flex", flexDirection:"column", height:"100%", overflow:"hidden" }}>
          ...
          return (
            <button key={item.id}
              ...
              style={{ display:"flex", alignItems:"center", gap:9, padding:"8px 10px", borderRadius:8, border:"none",
                background: active ? "rgba(99,102,241,.2)" : isNew ? "rgba(99,102,241,.08)" : "transparent",
                color: active ? "#818cf8" : isNew ? "#818cf8" : "#64748b",
      ...
      <div style={{ padding:"12px", borderTop:"1px solid rgba(255,255,255,.06)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
          ...
          <div style={{ minWidth:0 }}>
            <div style={{ fontSize:12, color:"#e2e8f0", fontWeight:600, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{user.name}</div>
            <div style={{ fontSize:11, color:"#475569", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{teamName}</div>
          </div>
        </div>
        <RoleBadge role={user.role} />
        <button onClick={onLogout} style={{ display:"block", marginTop:8, width:"100%", background:"transparent", border:"1px solid rgba(255,255,255,.07)", borderRadius:7, padding:"5px", color:"#475569", cursor:"pointer", fontSize:11, fontFamily:"inherit" }}>Cerrar sesión</button>
      </div>
    </aside>
  );
}
